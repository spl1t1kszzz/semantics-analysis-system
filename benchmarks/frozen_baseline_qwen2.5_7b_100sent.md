# ЗАФИКСИРОВАННЫЙ ЭТАЛОН (не редактировать)

Снимок baseline и первого multi-agent сравнения. Используется скриптом `compare_pipeline_multiagent.py` как frozen baseline.  
Актуальные прогоны: `results/compare_vs_frozen_baseline_*.md`.
